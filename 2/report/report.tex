...
\section{Motivation}
Although there exist many sophisticated image processing algorithms, extracting text lines from arbitrary formatted text documents is nowadays still an not generally solved issue. There are many problems that may arise when addressing this problem. The written text can either be hand-or/and machine written, the text can exhibit an unknown formatting (especially, when dealing with human handwritings) or the document itself might suffer from different types of artifacts or deformations. 

\section{Problem Satement}
In this work we describe an algorithm that allows us to extract text lines from different types of documents. In practise we are dealing with both, hand-and machine-writings. The extracted text lines may then used for further document analysis.

\section{Algorithm}
% TODO describe broadly the main steps

\subsection{Foreground/Background segmentation}
Given a certain document that exhibits some human and machine writings, our first task is to extract the contained/included text from its background. For the purpose of extracting the fore-and background of an image, we rely on a supervised binary image segmentation algorithm. This means, given some user specified cues that are labeling some pixels in image whether the belong to its fore-or background, we want to find a binary image s.t. each pixel is assigned to a label. This problem statement can be addressed by formulating the problem as an energy minimization problem. 

Our energy E depends on two terms: a data-and a smoothness-term. The smoothness term describes the desired relationship between neighboring pixels whereas the data term is encoding the probability for certain output pixel value (i.e. probability, that a pixel belongs to a certain label). The described optimization problem can be numerically solved by constructing an appropriate  weighted flow graph and then computing its minimum cut. This value corresponds to the minimal energy. This network graph is constructed as the following procedure describes:
\begin{itemize}
	\item For each pixel in the given image, we add a node to the graph. Two additional nodes are added to the graph, one for each binary label having the label F and B.
...
Using the described weighted flow graph, we can find an optimal labeling by solving for its minimum cut. A cut removes edges to disconnect the label F from the label Node B (i.e. there is no path from F to B). Thus, the min. cut is minimized sum of cut edge weights (i.e. min. energy of cut). The min cut can be determined by the ford \emph{Fulkerson's algorithm}. 
...
\section{Conclusion}
Resolve overlaps/conflicts for convex hulls. Also try to preprocess image to make adjust lines orthagonal to image boundaries.
