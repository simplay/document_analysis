...
to preprocess the distorted image to improve results.
\end{abstract}
...
\section{Algorithm}
The goal of our algorithm is to detect the type of shape contained in a provided binary valued image. Such a given image may suffer from either different types of noise or its shapes may exhibit fringes. \\
...
Since our image is binary valued we can detect Gaussian noise by computing the histogram of the image. If it has more than two bins (zero/one-valued bins) then we know there are other responses present in the image which is a characteristic property of gaussian noise. \\
 ...
 The Salt and Pepper noise can be corrected by applying a median filter onto an image. Therefore we simply apply a certain median filter onto the image and compute the difference between the input image and its median filtered version. In case the difference is too large we assume that there is Salt and Pepper noise present. \\

Finally for deciding whether an image suffers from fringes. Comparing a clean image with its fringe version, we notice that fringes are pixels along the edges that are wrong valued (holes and dents along shape edges). This we to extract the difference between a clean version of the image and the finge version by computing some border along the finge image. If the difference is too big, we say there are fringes present. \\
...
Lastly, If no of the previous checks have yielded any result, we assume that our image does not suffer from any kind of degeneration.   
  
\subsection{Preprocessing}
From the degeneration detection step we know what kind of issues we have to deal with. The simplest case occurs if no issue was detected. then our algorithm can skip this step and thus can directly start to detect corners in the image. \\

When fringes were detected this means that our image has degenerated edges. Such edges either exhibit holes or dents. 
% TODO show shape with different fringe levels and show what holes or dents actually are.
This is why we have to apply an edge-restoration. The idea is to smooth the edges out and then compute a new image by applying a thresholding. If a smoothed pixel is below some given threshold value then we say, that this pixel does not belong to the shape's edge and contrary otherwise. \\
...
\begin{figure}%
\centering
\subfigure[Noisy shape]{\includegraphics[width=0.1\textwidth]{triangle_noisy}}
