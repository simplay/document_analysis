...
\section{Algorithm}
The goal of our algorithm is to detect the type of shape contained in a provided binary valued, potentially noisy image. The shape can be arbitrary orientated and scaled. We only have to be able to detect the, triangles, squares, circles and a regular 5-corner star. The key idea of our algorithm is to detect all corners of a shape image. For this purpose we decided to implement a Harris corner detector. Such a detector works well for clean images. However, since the given input shape-images may suffer from either noise (Salt & Pepper noise) or also could exhibit degenerated edges we first have to apply a preprocessing step onto the input images before applying the corner detector. In summary our algorithm does the following for each given input image:
	\begin{itemize}
		\item remove potentially present noise
		\item restore degenerated shapes
...
\subsubsection{Restoration of edges}

\subsubsection{Pre smoothing for harris detector}
