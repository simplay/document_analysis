...
were exported using some other framework and focused on finding occurences within 
a given text line.

\section{Algorithm}
In a first step, we retrieve descriptors from all database images. 
The retrieved descriptors are then reduced to 50 types 
(or so called \emph{words}). For every image
we create then a representation using only the count of occurences of these 50 words.


When receiving a query image, we compute descriptors on it the same way as on a database image, 
then find for every descriptor the closest word and compare 
it in a last step with all images in the database.
...
\section{Results}
While our keyword spotting framework achieved very promising results in some cases when matching single words (See e. g. Figure \ref{fig:results_words:roxx}), very short words (Figure \ref{fig:results_words:suxx}) were troublesome until the very end. 
Additionally, results for the short words, worsened as we tried to generalize our method to finding keywords in full sentences instead of
matching only words.
\begin{figure}[ht!]%
\centering
...
our method does unfortunately not yield satisfying results. 
\begin{figure}[ht!]%
\centering
